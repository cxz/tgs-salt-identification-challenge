...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for d in [dec0, dec1, dec2, dec3, dec4]:
            print(d.size())
        ...
        print(F.upsample(dec1, scale_factor= 2, mode='nearest'))
        
        # hyper column
        # https://www.kaggle.com/c/tgs-salt-identification-challenge/discussion/64645
        f = torch.cat([
            dec0,
            F.upsample(dec1, scale_factor= 2, mode='nearest'),
            F.upsample(dec2, scale_factor= 4, mode='nearest'),
            F.upsample(dec3, scale_factor= 8, mode='nearest'),
            F.upsample(dec4, scale_factor=16, mode='nearest'),
        ], dim=1)
